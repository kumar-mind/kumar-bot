...
import ai.kumar.server.api.aaa.*;
import ai.kumar.server.api.cms.*;
import ai.kumar.server.api.kumar.*;
import ai.kumar.tools.MultipartConfigInjectionHandler;
...
import ai.kumar.server.HttpsMode;
import ai.kumar.server.RemoteAccess;
import ai.kumar.server.api.learning.ConsoleLearning;
import ai.kumar.server.api.service.EmailSenderService;
import ai.kumar.server.api.vis.MapServlet;
import ai.kumar.server.api.vis.MarkdownServlet;
import ai.kumar.server.api.vis.PieChartServlet;
import ai.kumar.tools.OS;
import org.glassfish.jersey.server.ResourceConfig;
import org.glassfish.jersey.servlet.ServletContainer;


public class KumarServer {
    ...
    public static ContextHandler buildSwaggerUI() throws Exception {
        ...
        rh.setResourceBase(KumarServer.class.getClassLoader()
    ...
    public static void main(String[] args) throws Exception {
        ...
        Log.getLog().info("Starting KUMAR initialization");

        // prepare shutdown signal
        File pid = new File(dataFile, "kumar.pid");
        if (pid.exists()) pid.deleteOnExit(); // thats a signal for the stop.sh script that KUMAR has terminated
        ...
        // check if a KUMAR service is already running on configured port
        try{
        ...
        KumarServer.server.start();
        KumarServer.caretaker = new Caretaker();
        KumarServer.caretaker.start();
        ...
        Runtime.getRuntime().addShutdownHook(new Thread() {
            public void run() {
                try {
                    ...
                    KumarServer.caretaker.shutdown();
                    KumarServer.server.stop();
                    DAO.close();
    ...
    private static void setupHttpServer(int httpPort, int httpsPort) throws Exception{
        ...
        KumarServer.server = new Server(pool);
        KumarServer.server.setStopAtShutdown(true);
        ...
        if(!httpsMode.equals(HttpsMode.ONLY)){
	        ...
	        ServerConnector connector = new ServerConnector(KumarServer.server);
	        ...
	        KumarServer.server.addConnector(connector);
        }
        ...
        if(httpsMode.isGreaterOrEqualTo(HttpsMode.ON)){
	        ...
	        //ServerConnector sslConnector = new ServerConnector(KUMARServer.server, ssl, alpn, http2, http1);
	        ServerConnector sslConnector = new ServerConnector(KumarServer.server, ssl, http1);
	        sslConnector.setPort(httpsPort);
	        sslConnector.setName("httpd:" + httpsPort);
	        sslConnector.setIdleTimeout(20000); // timout in ms when no bytes send / received
	        KumarServer.server.addConnector(sslConnector);
        }
    }
    ...
    private static void setServerHandler(File dataFile){
        ...
        if(redirect || auth){
            ...
            org.eclipse.jetty.security.LoginService loginService = new org.eclipse.jetty.security.HashLoginService("KUMARRealm", DAO.conf_dir.getAbsolutePath() + "/http_auth");
        	if(auth) KumarServer.server.addBean(loginService);
        ...
        services = new Class[]{
                ...
                // kumar search aggregation services
        ...
        // kumar api
        servletHandler.addServlet(UnansweredServlet.class, "/kumar/unanswered.txt");
        ...
        // aggregation api
        servletHandler.addServlet(GenericScraper.class, "/kumar/genericscraper.json");
        ...
        KumarServer.server.setSessionIdManager(idmanager);
        ...
        resourceConfig.packages(KumarServer.class.getPackage().getName(),
        ...
        KumarServer.server.setHandler(multipartConfigInjectionHandler);
    ...
    private static void checkServerPorts(int httpPort, int httpsPort) throws IOException{
        ...
        if(!httpsMode.equals(HttpsMode.ONLY)){
	        ...
	        try {
	            ...
	            throw new IOException("port " + httpPort + " is already occupied by another service, maybe another KUMAR is running on this port already. exit.");
	        } finally {
        ...
        if(httpsMode.isGreaterOrEqualTo(HttpsMode.ON)){
	        ...
	        try {
	            ...
	            // the socket is already occupied by another service
	        	throw new IOException("port " + httpsPort + " is already occupied by another service, maybe another KUMAR is running on this port already. exit.");
	        } finally {
	            // close the socket again
