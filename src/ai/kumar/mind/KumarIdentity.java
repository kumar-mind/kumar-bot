...
/**
 * Identity is the mental model of a being represented with a KumarMind.
 ...
 * the identity stores.
 */
public class KumarIdentity {

    private KumarAwareness long_term_memory, short_term_memory;
     ...
     */
    public KumarIdentity(File memorypath, int attention) {
        this.attention = attention;
        this.long_term_memory = new KumarAwareness();
        this.short_term_memory = new KumarAwareness();
        memorypath.mkdirs();
        this.memorydump = new File(memorypath, "log.txt");
        if (this.memorydump.exists()) {
            try {
                this.long_term_memory = KumarAwareness.readMemory(this.memorydump, attention);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
     ...
     * @return self
     */
    public KumarIdentity add(KumarCognition cognition) {
        this.short_term_memory.learn(cognition);
        List<KumarCognition> forgottenCognitions = this.short_term_memory.limitAwareness(this.attention);
     ...
     * @return self
     */
    public KumarIdentity setAttention(int attention) {
        this.attention = attention;
        return this;
    }
     ...
     */
    public List<KumarCognition> getCognitions() {
        ArrayList<KumarCognition> cognitions = new ArrayList<>();
