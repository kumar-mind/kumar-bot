...
public class KumarProcedures extends LinkedHashMap<Pattern, BiFunction<KumarArgument, Matcher, KumarThought>> implements Map<Pattern, BiFunction<KumarArgument, Matcher, KumarThought>> {

    private static final long serialVersionUID = 4531596762427825563L;

    public KumarProcedures() {
        super();
    }
     ...
     */
    public KumarThought deduce(KumarArgument flow, String q) {
        if (q == null) return new KumarThought();
        q = q.trim();
        for (Map.Entry<Pattern, BiFunction<KumarArgument, Matcher, KumarThought>> pe: this.entrySet()) {
            Pattern p = pe.getKey();
            Matcher m = p.matcher(q);
            if (new TimeoutMatcher(m).matches()) try {
                KumarThought json = pe.getValue().apply(flow, m);
                if (json != null) {
     ...
     * @return a thought from the application of the intent set
     */
    public KumarThought inspire(String q) {
