...
import ai.kumar.tools.MapTools;

/**
 * Kumars log is a kind of reflection about the conversation in the past
 */
public class KumarMemory {
    ...
    private Map<String, KumarIdentity> memories;
    private Map<String, Map<String, JsonTray>> intentsets;
    private Map<String, AtomicInteger> unanswered;
    
    public KumarMemory(File storageLocation, int attention) {
    ...
    public List<TokenMapList> unanswered2tokenizedstats() {
        ...
        this.unanswered.forEach((term, counter) -> {
            DAO.kumar.getReader().tokenizeSentence(term).forEach(token -> {
     ...
     */
    public List<KumarCognition> getCognitions(String client) {
        KumarIdentity identity = this.memories.get(client);
        if (identity == null) {
            if (root == null) return new ArrayList<KumarCognition>();
            identity = new KumarIdentity(new File(root, client), attention);
            this.memories.put(client, identity);
        }
        return identity.getCognitions();
    }
    
    public KumarMemory addCognition(String client, KumarCognition si) {
        KumarIdentity identity = this.memories.get(client);
        if (identity == null) {
     ...
     */
    public TreeMap<Long, KumarAwareness> getAllMemories() {
        TreeMap<Long, KumarAwareness> all = new TreeMap<>();
        ...
        for (String client: clients) {
            ...
            if (memorypath.exists()) {
                ...
                if (memorydump.exists()) try {
                    KumarAwareness awareness = KumarAwareness.readMemory(memorydump, Integer.MAX_VALUE);
