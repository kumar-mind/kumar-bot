...
import ai.kumar.DAO;
import ai.kumar.mind.KumarInference.Type;

/**
 * A kumar skill is a set of intents.
 * This class provides parser methods for such sets, given as text files
 */
public class KumarSkill {


    /**
     * read an "EzD" ('Easy Dialog') file: this is just a text file. Read the docs/kumar_skill_development_tutorial.md for an explanation
     * @param br
     * @return
            ...
            if (bang_type.length() > 0) {
                ...
                if (line.toLowerCase().equals("eol")) {
                    ...
                    if (bang_type.equals("javascript")) {
                        ...
                        for (String phrase: bang_phrases.split("\\|")) phrases.put(KumarPhrase.simplePhrase(phrase.trim(), prior));
                        ...
                        intent.put("actions", new JSONArray().put(KumarAction.answerAction(bang_term.split("\\|"))));
                        ...
                        if (bo.has("url") && bo.has("path")) {
                            ...
                            if (bang_term.length() > 0) {
                                actions.put(KumarAction.answerAction(bang_term.split("\\|")));
                            }
                            ...
                            if (bo.has("actions")) {
                                ...
                                bo_actions.forEach(action -> {
                                    ...
                                    if (type.equals(KumarAction.RenderType.table.toString()) && boa.has("columns")) {
                                        actions.put(KumarAction.tableAction(boa.getJSONObject("columns"),
                                                    boa.has("length") ? boa.getInt("length") : -1));
                                    } else
                                    if (type.equals(KumarAction.RenderType.piechart.toString()) &&
                                            boa.has("total") && boa.has("key") &&
                                            boa.has("value") && boa.has("unit")) {
                                        actions.put(KumarAction.piechartAction(
                                                boa.getInt("total"), boa.getString("key"),
                                                boa.getString("value"), boa.getString("unit")));
                                    } else
                                    if (type.equals(KumarAction.RenderType.rss.toString()) &&
                                            boa.has("title") && boa.has("description") && boa.has("link")) {
                                        actions.put(KumarAction.rssAction(
                                            boa.getString("title"), boa.getString("description"), boa.getString("link"),
                                            boa.has("length") ? boa.getInt("length") : -1));
                                    } else
                                    if (type.equals(KumarAction.RenderType.websearch.toString()) && boa.has("query")) {
                                        actions.put(KumarAction.websearchAction(boa.getString("query")));
                                    } else
                                    if (type.equals(KumarAction.RenderType.map.toString()) &&
                                            boa.has("latitude") && boa.has("longitude") && boa.has("zoom")) {
                                        actions.put(KumarAction.mapAction(
                                            boa.getDouble("latitude"), boa.getDouble("longitude"), boa.getInt("zoom")));
                                    } else
                                    if(type.equals(KumarAction.RenderType.timer_set.toString()) &&
                                            boa.has("hour")){
                                        int hour = boa.getInt("hour");
                                            actions.put(KumarAction.timerSetAction(
                ...
                if (line.startsWith("?") && (thenpos = line.indexOf(':')) > 0) {
                    ...
                    if (elsepos <= thenpos) {
                        ...
                        if (ifsubstring.length() > 0) {
                            ...
                            JSONObject intent = KumarIntent.answerIntent(phrases, "IF " + condition, answers, prior, example, expect);
                            intents.put(intent);
                        }
                    } else {
                        ...
                        if (ifsubstring.length() > 0) {
                            ...
                            JSONObject intentif = KumarIntent.answerIntent(phrases, "IF " + condition, ifanswers, prior, example, expect);
                        ...
                        if (elsesubstring.length() > 0) {
                            ...
                            JSONObject intentelse = KumarIntent.answerIntent(phrases, "NOT " + condition, elseanswers, prior, example, expect);
                    ...
                    JSONObject intent = KumarIntent.answerIntent(phrases, condition, answers, prior, example, expect);
    ...
    public static JSONObject readAIMLCategory(Node category) {
        ...
        if (phrases != null && answers != null) {
            return KumarIntent.answerIntent(phrases, null, answers, false, null, null);
        }
        return null;
    }
