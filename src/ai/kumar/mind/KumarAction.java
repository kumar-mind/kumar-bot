...
public class KumarAction {
     ...
     * @param json
     */
    public KumarAction(JSONObject json) {
        this.json = json;
    }
     ...
     */
    public KumarAction execution(KumarArgument thoughts, KumarMind mind, String client, KumarLanguage language) {
        if ((this.getRenderType() == RenderType.answer || this.getRenderType() == RenderType.self) && this.json.has("phrases")) {
            ...
            if (expression != null) {
                ...
                // self-referrer evaluate contents from the answers expressions as recursion: kumar is asked again
                while (new TimeoutMatcher(m = self_referrer.matcher(expression)).matches()) {
                    String observation = m.group(1);
                    KumarMind.Reaction reaction = mind.new Reaction(observation, language, client, new KumarThought());
                ...
                while (new TimeoutMatcher(m = visible_assignment.matcher(expression)).matches()) {
                    ...
                    thoughts.think(new KumarThought().addObservation(variable, observation));
                }
                ...
                if (this.getRenderType() == RenderType.self) {
                    // recursive call kumar with the answer
                    KumarMind.Reaction reaction = mind.new Reaction(expression, language, client, new KumarThought());
