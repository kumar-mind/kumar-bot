 ...
 */
public class KumarAwareness {

    private final ArrayList<KumarCognition> aware;
    
    public KumarAwareness() {
     ...
     * @return self
     */
    public KumarAwareness learn(KumarCognition cognition) {
     ...
     */
    public KumarCognition forgetOldest() {
     ...
     */
    public List<KumarCognition> limitAwareness(int attention) {
        ArrayList<KumarCognition> removed = new ArrayList<>();
        while (attention != Integer.MAX_VALUE && this.getTime() > attention) {
            KumarCognition c = this.forgetOldest();
            if (c == null) break;
            removed.add(0, c);
        }
        return removed;
     ...
     */
    public KumarCognition getLatest() {
     ...
     * @throws IOException
     */
    public static KumarAwareness readMemory(final File memorydump, int attentionTime) throws IOException {
        List<String> lines = Files.readAllLines(memorydump.toPath());
        KumarAwareness awareness = new KumarAwareness();
        for (int i = lines.size() - 1; i >= 0; i--) {
            String line = lines.get(i);
            if (line.length() == 0) continue;
            KumarCognition si = new KumarCognition(new JSONObject(line));
