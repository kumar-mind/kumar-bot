 ...
 * Within the Kumar AI infrastructure this may be considered as the representation of
 * the short-time memory of thinking inside Kumar.
 */
public class KumarArgument implements Iterable<KumarThought> {

    private final ArrayList<KumarThought> recall;
     ...
     */
    public KumarArgument() {
        this.recall = new ArrayList<>();
        this.actions = new ArrayList<>();
        this.skills = new ArrayList<>();
    }
    
    public KumarArgument clone() {
        KumarArgument c = new KumarArgument();
     ...
     */
    public KumarThought mindmeld(boolean reverse) {
        KumarThought meltedMind = new KumarThought();
        if (reverse)
     ...
     */
    public KumarThought remember(int timesBack) {
        int state = this.recall.size() - timesBack - 1;
        if (state < 0) return new KumarThought(); // empty mind!
     ...
     */
    public KumarThought rethink() {
        if (this.recall.size() <= 0) return new KumarThought(); // empty mind!
        KumarThought rethought = this.recall.remove(this.recall.size() - 1);
        return rethought;
    }
     ...
     */
    public KumarArgument think(KumarThought thought) {
     ...
     * @return self, the current argument
     */
    public KumarArgument think(KumarArgument argument) {
    ...
    public String unify(String statement, boolean urlencode, int depth) {
        for (KumarThought t: this) {
            ...
            if (!new TimeoutMatcher(KumarThought.variable_pattern.matcher(statement)).find()) return statement;
        }
        if (new TimeoutMatcher(KumarThought.variable_pattern.matcher(statement)).find()) return null; // failure!
        return statement;
    }
     ...
     */
    @Override
    public Iterator<KumarThought> iterator() {
        return new Iterator<KumarThought>() {
            private int p = recall.size(); 
            @Override public boolean hasNext() {return p > 0;}
            @Override public KumarThought next() {return recall.get(--p);}
        };
    }
     ...
     * @return the argument
     */
    public KumarArgument addAction(final KumarAction action) {
     ...
     * @return the argument
     */
    public KumarArgument addSkill(final String skill) {
     ...
     */
    public KumarThought finding(KumarMind mind, String client, KumarLanguage language) {
        ...
        KumarThought answer = this.mindmeld(true);
     ...
     */
    public List<KumarAction> getActions() {
        return this.actions;
    }
    ...
    public static void main(String[] args) {
        KumarArgument a = new KumarArgument().think(new KumarThought().addObservation("a", "letter-a"));
