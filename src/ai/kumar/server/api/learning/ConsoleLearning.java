...
package ai.kumar.server.api.learning;
 ...
 * {
  ...
  "path_computed": ["$.statuses"],
  "console": {"loklak": {
    "example": "http://127.0.0.1:4000/kumar/console.json?q=%22SELECT%20*%20FROM%20loklak%20WHERE%20query=%27fossasia%27;%22",
...
public class ConsoleLearning extends AbstractAPIHandler implements APIHandler {
    ...
    public ServiceResponse serviceImpl(Query post, HttpServletResponse response, Authorization user, final JsonObjectWithDefault permissions) throws APIException {
        ...
        if (action.equals("list")) {
            Set<String> projectnames = DAO.kumar.getIntentsetNames(client);
            JSONObject projects = new JSONObject(true);
            for (String p: projectnames) {
                try {
                    JsonTray t = DAO.kumar.getIntentset(client, p);
            ...
            if (action.equals("delete")) {
                try {
                    JsonTray tray = DAO.kumar.getIntentset(client, project);
            ...
            consolerule.put("example", "http://127.0.0.1:4000/kumar/console.json?q=%22SELECT%20*%20FROM%20" + name + "%20WHERE%20query=%27" + test + "%27;%22");
