...
public class BahnService {
	...
	public KumarThought getConnections(String from, String to) throws IOException, NoStationFoundException {
		Calendar cal = Calendar.getInstance();
		return this.getConnections(from, to, cal.get(Calendar.HOUR_OF_DAY), cal.get(Calendar.MINUTE));
	}
	
	@SuppressWarnings("deprecation")
	public KumarThought getConnections(String from, String to, int hoursDep, int minutesDep) throws IOException, NoStationFoundException {
		// Yes, Date is deprecated, but the referenced library needs it...
		Date date = new Date();
		date.setHours(hoursDep);
		date.setMinutes(minutesDep);
		KumarThought response = new KumarThought();
