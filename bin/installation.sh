...
echo "starting kumar installation"
echo "startup" > $STARTUPFILE

cmdline="$cmdline -server -classpath $CLASSPATH -Dlog4j.configurationFile=$LOGCONFIG ai.kumar.KumarInstallation >> data/kumar.log 2>&1 &";
...
if [ -f $STARTUPFILE ] && [ $(ps -p $PID -o pid=) ]; then
	...
	echo "kumar installation started at port $CUSTOMPORT, open your browser at $LOCALHOST"
	rm -f $STARTUPFILE

    echo "waiting for installation to finish"
    wait "$PID"
    if [ $? -eq 0 ]; then
        echo "kumar installation finished"
        echo 'done' > $INSTALLATIONCONFIG
    else
        echo "kumar installation aborted"
    fi

	exit 0
else
	echo "kumar installation failed to start. See data/loklag.log for details. Here are the last logs:"
    tail data/kumar.log
	rm -f $STARTUPFILE
	exit 1
fi
