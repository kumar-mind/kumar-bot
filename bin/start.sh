...
while getopts ":Idn" opt; do
    case $opt in
        ...
        \?)
            ...
            echo -e " -d\tSkip waiting for Kumar"
...
if [ ! -f $INSTALLATIONCONFIG ] && [[ $SKIP_INSTALL_CHECK -eq 0 ]]; then
    echo "kumar detected that you did not yet run the installation wizard."
...
cmdline="$cmdline -server -classpath $CLASSPATH -Dlog4j.configurationFile=$LOGCONFIG ai.kumar.KumarServer";
...
cmdline="$cmdline >> data/kumar.log 2>&1 &"
...
if [ -f $STARTUPFILE ] && [ $(ps -p $PID -o pid=) ]; then
	...
	echo "kumar server started at port $CUSTOMPORT, open your browser at $LOCALHOST"
	rm -f $STARTUPFILE
	exit 0
else
	echo "kumar server failed to start. See data/kumar.log for details. Here are the last logs:"
    tail data/kumar.log
	rm -f $STARTUPFILE
	exit 1
fi
